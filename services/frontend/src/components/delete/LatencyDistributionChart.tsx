...
import type { DeleteResultResponse } from "@/types/delete"
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between gap-3">
          <CardTitle className="text-base inline-flex items-center gap-2">
            ...
            Engine-side delete latency per DB — p50 / p95 / p99 (ms)
          </CardTitle>
          <DownloadChartButton containerRef={chartRef} chartName="delete-latency" />
        </div>
      </CardHeader>
      <CardContent>
        {data.length === 0 ? (
          <EmptyState
            ...
            description="Delete latency will appear here as the run progresses."
          />
        ) : (
          <div ref={chartRef} className="h-72 bg-white rounded-sm">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Bar dataKey="p99" name="p99" fill="#a855f7" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
  ...
  return (
    <div className="rounded-md border border-border bg-background p-2 shadow-md text-xs space-y-0.5">
      ...
      <div className="text-muted-foreground mt-1">
        {r.samples} samples · {r.rows} deleted
      </div>
    </div>
  )
}
