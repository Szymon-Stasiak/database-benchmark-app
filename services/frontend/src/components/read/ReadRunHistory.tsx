...
interface Props {
  runs: ReadRunResponse[]
  selectedRunId: string | null
  onSelect: (run: ReadRunResponse) => void
}

const STATUS_ICON: Record<ReadStatus, React.ComponentType<{ className?: string }>> = {
  PENDING: Clock,
  RUNNING: Loader2,
...
export function ReadRunHistory({ runs, selectedRunId, onSelect }: Props) {
  if (runs.length === 0) {
    return (
      <Card>
        <CardHeader>
