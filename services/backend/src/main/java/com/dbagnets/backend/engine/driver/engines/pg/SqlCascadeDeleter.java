...
public final class SqlCascadeDeleter {
    ...
    public interface Binder {
        void bind(PreparedStatement stmt, int index, LogicalAttribute attr, Object value)
                throws SQLException;
    }
    ...
    public static int deleteRoot(
        ...
        String sql =
                "DELETE FROM "
                        + dialect.quote().apply(entity.name().toLowerCase())
                        + " WHERE "
                        + dialect.quote().apply(pk.name().toLowerCase())
                        + " = ?";
    ...
    public static int deleteRootBatch(
            Connection conn,
            LogicalEntity entity,
            LogicalAttribute pk,
    ...
    public static int deleteRootBulk(
            Connection conn,
            LogicalEntity entity,
            LogicalAttribute pk,
            List<Object> physicalIds,
    ...
    public static Map<String, List<Object>> cascadeChildrenOf(
            Connection conn,
        ...
        while (state.hasNext()) {
            ...
            for (LogicalRelationship rel : schema.relationships()) {
                ...
                LogicalAttribute parentPk =
                        schema.findEntity(rel.parentEntity())
                                .flatMap(LogicalEntity::primaryKey)
                                .orElse(null);
                if (parentPk == null) continue;

                List<Object> childIds =
                        selectChildIds(conn, child, childPk, fkCol, parentPk, curIds, dialect);
    ...
    private static List<Object> selectChildIds(
            Connection conn,
            LogicalEntity child,
        ...
        String sql =
                "SELECT "
                        + dialect.quote().apply(childPk.name().toLowerCase())
                        + " FROM "
                        + dialect.quote().apply(child.name().toLowerCase())
                        + " WHERE "
                        + dialect.quote().apply(fkCol.toLowerCase())
                        + " = ?";
