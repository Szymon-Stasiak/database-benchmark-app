...
public final class FakerCatalog {
    ...
    private static final double NULL_PROBABILITY =
            0.10; // to make data realistic if nullable you can get null
    ...
    public Object generate(LogicalAttribute attr) {
        if (!attr.isPrimaryKey()
                && attr.isNullable()
                && ThreadLocalRandom.current().nextDouble() < NULL_PROBABILITY) {
            return null;
        }
    ...
    private boolean isTextLike(LogicalDataType type) {
        return type == LogicalDataType.STRING
                || type == LogicalDataType.TEXT
                || type == LogicalDataType.JSON;
    }
    ...
    private Function<LogicalAttribute, Object> matchByName(String name) {
        ...
        if (lower.contains("full_name") || lower.equals("name"))
            return a -> faker.name().fullName();
        ...
        if (lower.contains("avatar") || lower.contains("image"))
            return a -> faker.internet().image();
        if (lower.contains("title")) return a -> faker.book().title();
        if (lower.contains("biography")
                || lower.contains("bio")
                || lower.contains("synopsis")
                || lower.contains("description")
                || lower.contains("body")) return a -> faker.lorem().paragraph();
        if (lower.contains("nationality") || lower.contains("country"))
            return a -> faker.country().name();
        ...
        if (lower.contains("tags"))
            return a -> "[\"" + faker.lorem().word() + "\",\"" + faker.lorem().word() + "\"]";
        return null;
    }
    ...
    private Object generateByType(LogicalAttribute attr) {
        return switch (attr.dataType()) {
            ...
            case DATE ->
                    LocalDate.now().minusDays(ThreadLocalRandom.current().nextInt(0, 365 * 50));
            case TIMESTAMP ->
                    Instant.now()
                            .minus(
                                    ThreadLocalRandom.current().nextLong(0, 365L * 24),
                                    ChronoUnit.HOURS);
            case JSON ->
                    "{\"key\":\""
                            + faker.lorem().word()
    ...
    private BigDecimal generateDecimal(LogicalAttribute attr) {
        ...
        double value =
                ThreadLocalRandom.current()
                        .nextDouble(0.0, Math.pow(10, Math.max(0, precision - scale)));
