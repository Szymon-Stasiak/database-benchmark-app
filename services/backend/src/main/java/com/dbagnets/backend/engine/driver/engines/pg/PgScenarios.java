...
public final class PgScenarios {
    ...
    public static String buildAggregateSql(
            LogicalSchema schema, String parentEntity, String childEntity) {
        LogicalRelationship rel =
                ScenarioSupport.findRelationship(schema, parentEntity, childEntity);
        String childTable = quote(childEntity.toLowerCase());
        String fkCol = quote(rel.fkColumnInChild().toLowerCase());
        return "SELECT "
                + fkCol
                + " AS group_key, COUNT(*) AS cnt FROM "
                + childTable
                + " WHERE "
                + fkCol
                + " IS NOT NULL"
    ...
    public static Map<String, Long> executeAggregate(
            Connection conn, LogicalSchema schema, String parentEntity, String childEntity)
            throws SQLException {
    ...
    public static String buildRangeSql(LogicalSchema schema, String entityName, String attribute) {
        ...
        LogicalAttribute attr =
                entity.findAttribute(attribute)
                        .orElseThrow(
                                () ->
    ...
    public static long executeRangeCount(
            Connection conn,
    ...
    public static List<TraversalLevel> resolveChain(
            LogicalSchema schema, String startEntity, int depth) {
        List<TraversalLevel> chain = new ArrayList<>();
        String current = startEntity;
        for (int level = 1; level <= depth; level++) {
            final String parent = current;
            LogicalRelationship next =
    ...
    public static String buildTraversalSql(LogicalSchema schema, String startEntity, int depth) {
        ...
        LogicalAttribute startPk =
                start.primaryKey()
                        .orElseThrow(
                                () ->
                                        new IllegalStateException(
                                                startEntity + " has no primary key"));
        List<TraversalLevel> chain = resolveChain(schema, startEntity, depth);
        if (chain.isEmpty()) {
            return "SELECT CAST(NULL AS TEXT) AS id WHERE FALSE";
        }

        StringBuilder cteBlock =
                new StringBuilder("WITH walk_0 AS (")
                        .append("SELECT ")
                        .append(quote(startPk.name().toLowerCase()))
                        .append(" AS id ")
                        .append("FROM ")
                        .append(quote(start.name().toLowerCase()))
                        .append(" WHERE ")
                        .append(quote(startPk.name().toLowerCase()))
                        .append(" = ?")
                        .append(")");
        ...
        for (TraversalLevel level : chain) {
            ...
            LogicalAttribute childPk =
                    childEntity
                            ...
                            .orElseThrow(
                                    () ->
                                            new IllegalStateException(
                                                    level.childEntity() + " has no primary key"));
            ...
            cteBlock.append(", ")
                    ...
                    .append(" AS (")
                    .append("SELECT child.")
                    .append(quote(childPk.name().toLowerCase()))
                    .append(" AS id ")
                    .append("FROM ")
                    .append(quote(childEntity.name().toLowerCase()))
                    .append(" child")
                    .append(" JOIN ")
                    .append(prevCte)
                    .append(" parent ON child.")
        ...
        return cteBlock
                + " SELECT reachable_id FROM ("
                + unionBlock
                + ") reached ORDER BY reachable_id";
    }

    public static List<String> executeTraversal(
            Connection conn,
            LogicalSchema schema,
            String startEntity,
    ...
    public record TraversalLevel(
            int depth, String parentEntity, String childEntity, String fkColumn) {}
}
