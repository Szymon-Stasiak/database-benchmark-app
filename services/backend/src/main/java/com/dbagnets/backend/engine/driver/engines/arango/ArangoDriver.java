package com.dbagnets.backend.engine.driver.engines.arango;
...
public class ArangoDriver implements EngineDriver {
    ...
    public TimedOperation read(ReadContext ctx) {
        ...
        return PerTargetLoop.run(
                ctx.targets(),
                "Arango read",
                e -> collection + "/" + e.physicalId(),
                entry -> {
                    JsonNode resp =
                            client.get()
                                    .uri(
                                            "/_db/_system/_api/document/{c}/{k}",
                                            collection,
                                            entry.physicalId())
                                    .retrieve()
    ...
    private com.dbagnets.backend.engine.driver.api.EntityOutcome bulkInsert(
            WebClient client,
            InsertContext ctx,
            com.dbagnets.backend.engine.cascade.CascadeNode node,
        ...
        return BulkInsertLoop.run(
                ...
                (slice, batchIndex, totalBatches) -> {
                    List<Map<String, Object>> docs =
                            slice.stream()
                                    .map(row -> DocBuilders.withPkAsString("_key", entity, row))
                                    .toList();
                    client.post()
                            .uri(
                                    uri ->
                                            uri.path("/_db/_system/_api/document/{c}")
