...
public class ScenarioRunOrchestrator extends AbstractRunOrchestrator {
    ...
    @Override
    protected String runStatusEventName() {
        return SseEvents.EVENT_SCENARIO_RUN_STATUS;
    }

    @Override
    protected String resultStatusEventName() {
        return SseEvents.EVENT_SCENARIO_RESULT_STATUS;
    }

    @Override
    protected Object toResultResponse(BenchmarkResult result) {
        return ScenarioResultResponse.from(result);
    }
    ...
    protected void broadcastRunStatus(String benchmarkId, BenchmarkRun run) {
        sse.sendEvent(
                ...
                Map.of(
                        ...
                        "consistencyStatus",
                                run.getScenarioConsistencyStatus() == null
                                        ? ""
                                        : run.getScenarioConsistencyStatus()));
    }

    @Transactional
    public PreparedScenarioRunResponse prepareRun(
            String benchmarkId, StartScenarioRunRequest request) {
        Benchmark benchmark =
                benchmarkRepository
        ...
        for (String databaseId : request.databaseIds()) {
            ...
            applicability.add(
                    new PreparedScenarioRunResponse.Applicability(
                            db.getId(),
                            db.getDbName(),
                            applicable,
                            applicable
        ...
        sse.sendEvent(
                benchmarkId,
                SseEvents.EVENT_SCENARIO_RUN_PREPARED,
                Map.of("runId", run.getId(), "applicability", applicability));

        return new PreparedScenarioRunResponse(
                run.getId(), benchmarkId, type.name(), run.getStatus().name(), applicability);
    }
    ...
    public void confirmRun(String runId) {
        ...
        if (run.getStatus() != RunStatus.PENDING) {
            throw new IllegalStateException(
                    "Run " + runId + " is not in PENDING state (status=" + run.getStatus() + ")");
        }
        ...
        runDatabaseOperation(
                ...
                ctx -> {
                    ...
                    for (int i = 0; i < iterations; i++) {
                        EngineDriver.ScenarioOutcome outcome =
                                ctx.driver().runScenario(scenarioCtx);
    ...
    private void validateRequest(StartScenarioRunRequest request) {
        ...
        if (request.params() instanceof TraversalParams tp
                && (tp.startLogicalId() == null || tp.startLogicalId().isBlank())) {
            throw new IllegalArgumentException("startLogicalId is required for GRAPH_TRAVERSAL");
        }
        if (request.params() instanceof KnnParams kp
                && (kp.queryVector() == null || kp.queryVector().length == 0)) {
