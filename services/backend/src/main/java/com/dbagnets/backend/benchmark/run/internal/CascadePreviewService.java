...
public class CascadePreviewService {
    ...
    public CascadePreviewResponse previewFromRequest(LogicalSchema schema, CascadePreviewRequest request) {
    ...
    private CascadePlan planFrom(LogicalSchema schema, List<EntityCascadeChoiceDto> entities) {
        List<LeafChoice> leaves = entities.stream()
                .map(e -> new LeafChoice(e.entityName(), e.recordCount())).toList();
    ...
    private CascadePreviewResponse buildResponse(LogicalSchema schema, CascadePlan plan, CascadePreviewRequest request) {
        ...
        for (CascadeNode node : plan.nodesInInsertOrder()) {
            for (CascadeEdge edge : node.incomingFromParents()) {
                ...
                double defaultRatio = rel == null ? edge.parentToChildRatio() : rel.cardinality().defaultRatio();
                edges.add(new CascadePreviewResponse.CascadePreviewEdge(
                        edge.childEntity(), edge.parentEntity(), cardinality, defaultRatio,
                        edge.parentToChildRatio(), edge.fkColumnInChild()));
            }
        }
