...
import com.dbagnets.backend.engine.driver.support.CascadeBfsState;
...
import com.dbagnets.backend.engine.driver.support.ScenarioTimings;
...
public abstract class BoltDriverBase implements EngineDriver {
    ...
    public TimedOperation insert(InsertContext ctx) {
        org.neo4j.driver.Driver neo =
                driverCache.get(ctx.databaseId(), ctx.hostAddress(), ctx.hostPort());
    ...
    public TimedOperation read(ReadContext ctx) {
        ...
        com.dbagnets.backend.engine.driver.api.ReadDepth depth =
                ctx.readDepth() == null
                        ? com.dbagnets.backend.engine.driver.api.ReadDepth.NONE
                        : ctx.readDepth();
        String cypher;
        if (depth == com.dbagnets.backend.engine.driver.api.ReadDepth.NONE) {
            cypher = "MATCH (n:`" + label + "` {" + pk + ": $id}) RETURN n";
        } else {
            int maxDepth =
                    depth == com.dbagnets.backend.engine.driver.api.ReadDepth.ONE_HOP
                            ? 1
                            : com.dbagnets.backend.engine.driver.api.ReadDepth
                                    .FULL_CASCADE_MAX_DEPTH;
            var chain =
                    com.dbagnets.backend.engine.driver.engines.pg.PgScenarios.resolveChain(
            ...
            if (chain.isEmpty()) {
                ...
                cypher =
                        "MATCH (n:`"
                                ...
                                + chain.size()
                                + "]->(m) "
                                + "WHERE m IS NULL OR any(lbl IN labels(m) WHERE lbl IN ["
                                + labelFilter
                                + "]) "
        ...
        boolean cascading =
                depth != com.dbagnets.backend.engine.driver.api.ReadDepth.NONE
    ...
    public TimedOperation delete(DeleteContext ctx) {
        ...
        java.util.Map<String, java.util.List<String>> cascadeAccumulator =
        ...
        try (Session session = neo.session()) {
            for (int i = 0; i < ctx.targets().size(); i++) {
                ...
                if (ctx.includeChildren()) {
                    cascadeChildrenBfs(
                            session, ctx.schema(), label, entry.physicalId(), cascadeAccumulator);
                }
    ...
    private void cascadeChildrenBfs(
            Session session,
        ...
        while (state.hasNext()) {
            ...
            if (state.isNotVisited(cur)) continue;
        ...
        for (String entityName : state.reversedEntityOrder()) {
            ...
            try {
                ...
                accumulator
                        .computeIfAbsent(entityName, k -> new java.util.ArrayList<>())
    ...
    private EntityOutcome createNodes(
            Session session, InsertContext ctx, CascadeNode node, List<GeneratedRow> rows) {
                ...
                slice.forEach(
                        r ->
                                outcome.recordedIds.add(
                                        new RecordedId(
                                                node.entityName(), r.logicalId(), r.logicalId())));
            } catch (Exception ex) {
    ...
    private long createRelationships(
        ...
        for (int from = 0; from < pairs.size(); from += chunkSize) {
            ...
            try {
                ...
                log.warn(
                        "{} relationship MERGE failed {} -> {} chunk [{}, {}): {}",
                        engine(),
                        parentLabel,
                        childLabel,
                        from,
    ...
    private static String getString(InsertContext ctx, String parentLabel, String childLabel) {
        ...
        String childPk =
                requiredPkName(ctx.schema(), childLabel, "Child entity missing PK: " + childLabel);
        String parentPk =
                requiredPkName(
                        ctx.schema(), parentLabel, "Parent entity missing PK: " + parentLabel);

        return "UNWIND $pairs AS pair "
                + "MATCH (p:`"
                + parentLabel
                + "` {"
                + parentPk
    ...
    public ScenarioOutcome runScenario(ScenarioContext ctx) throws Exception {
        org.neo4j.driver.Driver neo =
                driverCache.get(ctx.databaseId(), ctx.hostAddress(), ctx.hostPort());
        try (Session session = neo.session()) {
            return ScenarioTimings.execute(
                    () ->
                            switch (ctx.params()) {
                                case AggregateParams p -> {
                                    Map<String, Long> grouped =
                                            BoltScenarios.executeAggregate(
                                                    session,
                                                    ctx.schema(),
                                                    p.parentEntity(),
                                                    p.childEntity());
                                ...
                                case RangeParams p -> {
                                    long count =
                                            BoltScenarios.executeRangeCount(
                                                    session,
                                                    ctx.schema(),
                                                    p.entityName(),
                                                    p.attribute(),
                                ...
                                case TraversalParams p -> {
                                    List<String> ids =
                                            BoltScenarios.executeTraversal(
                                                    session,
                                                    ctx.schema(),
                                                    p.startEntity(),
                                                    p.startLogicalId(),
    ...
    private static String pkName(
            com.dbagnets.backend.engine.schema.LogicalSchema schema, String entityName) {
        return schema.findEntity(entityName)
                .flatMap(LogicalEntity::primaryKey)
                .map(LogicalAttribute::name)
                .orElse(null);
    }

    private static String requiredPkName(
            com.dbagnets.backend.engine.schema.LogicalSchema schema,
            String entityName,
            String errorMessage) {
        return schema.requireEntity(entityName)
                .primaryKey()
