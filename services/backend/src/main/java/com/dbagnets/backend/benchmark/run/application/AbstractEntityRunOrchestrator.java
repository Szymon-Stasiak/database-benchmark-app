...
public abstract class AbstractEntityRunOrchestrator extends AbstractRunOrchestrator {
    ...
    public record EntityDatabaseContext(
            BenchmarkDatabase db,
            EngineDriver driver,
            EmbeddingMap embeddings,
            List<RegistryEntry> targets) {}

    protected Optional<EntityDatabaseContext> resolveEntityContext(
            String benchmarkId,
            String runId,
            String resultId,
            String entityName,
        ...
        Optional<List<RegistryEntry>> targetsOpt =
                runSupport.resolveTargets(
                        ctxOpt.get().db().getId(),
                        entityName,
                        selectedLogicalIds,
                        resultId,
                        resultBroadcast(benchmarkId, runId));
        return targetsOpt.map(
                registryEntries ->
                        new EntityDatabaseContext(
                                ctxOpt.get().db(),
                                ctxOpt.get().driver(),
                                ctxOpt.get().embeddings(),
                                registryEntries));
    }
    ...
    protected void markStartedWithSize(
            String benchmarkId, String runId, String resultId, Long sizeBefore) {
    ...
    protected void runEntityOperation(
            String benchmarkId,
            String runId,
            String resultId,
            String entityName,
        ...
        Optional<EntityDatabaseContext> ctxOpt =
                resolveEntityContext(benchmarkId, runId, resultId, entityName, selectedLogicalIds);
