...
public class BenchmarkDeploymentService {
        ...
        executor.submit(
                () -> {
                    try {
                        ...
                        log.info(
                                "Benchmark {} modified concurrently during orchestration, aborting silently",
                                benchmarkId);
                    } catch (Exception e) {
        ...
        executor.submit(
                () -> {
                    try {
                        ...
                        log.info(
                                "Benchmark {} modified concurrently during bundle import, aborting silently",
                                benchmarkId);
                    } catch (Exception e) {
        ...
        executor.submit(
                () -> {
                    try {
                        ...
                        log.info(
                                "Benchmark {} modified concurrently during redeploy, aborting silently",
                                benchmarkId);
                    } catch (Exception e) {
        ...
        executor.submit(
                () -> {
                    try {
                        ...
                        log.info(
                                "Database {} modified concurrently during redeploy, aborting silently",
                                databaseId);
                    } catch (Exception e) {
                        log.error(
                                "Redeploy failed for database {} in benchmark {}",
                                databaseId,
                                benchmarkId,
                                e);
    ...
    public void updateBenchmarkStatus(String benchmarkId, BenchmarkStatus status) {
        ...
        if (opt.isEmpty()) {
            log.info(
                    "Benchmark {} no longer exists, skipping status update to {}",
                    benchmarkId,
                    status);
            return;
        }
        ...
        try {
            ...
            log.info(
                    "Benchmark {} modified concurrently, skipping status update to {}",
                    benchmarkId,
                    status);
            return;
        }
    ...
    public void updateDatabaseStatus(String databaseId, DatabaseStatus status) {
        ...
        if (opt.isEmpty()) {
            log.info(
                    "Database {} no longer exists, skipping status update to {}",
                    databaseId,
                    status);
            return;
        }
        ...
        try {
            ...
            log.info(
                    "Database {} modified concurrently, skipping status update to {}",
                    databaseId,
                    status);
            return;
        }
    ...
    public void finalizeBenchmark(String benchmarkId) {
        ...
        boolean anyReadyOrStopped =
                benchmark.getDatabases().stream()
                        .anyMatch(
                                db ->
                                        db.getStatus() == DatabaseStatus.SCRIPT_READY
    ...
    private void generateScripts(String benchmarkId) {
        ...
        ScriptCreatorResponse response =
                scriptGenerator.generate(benchmark.getTopic(), benchmark.getDepth(), targets);
        ...
        if (!response.success()) {
            log.warn(
                    "Partial script-generation success: {}/{} databases produced a script — failed ones will be marked FAILED, the rest will proceed",
                    successCount,
                    response.scripts().size());
        }
        ...
        for (ScriptResult scriptResult : response.scripts()) {
            ...
            benchmark.getDatabases().stream()
                    .filter(
                            db ->
                                    db.getDbName().equals(scriptResult.dbName())
        ...
        for (BenchmarkDatabase db : benchmark.getDatabases()) {
            if (db.getStatus() == DatabaseStatus.SCRIPT_GENERATING) {
                ...
                databaseRepository.save(db);
                events.broadcastDatabaseStatus(
                        benchmarkId, db.getId(), DatabaseStatus.FAILED, "Script generation failed");
            }
        }
    }

    private void applyGeneratedScript(
            String benchmarkId, BenchmarkDatabase db, ScriptResult scriptResult) {
        ...
        if (!scriptResult.embeddingMappings().isEmpty()) {
            try {
                db.setEmbeddingMappings(
                        objectMapper.writeValueAsString(scriptResult.embeddingMappings()));
            } catch (Exception e) {
                log.warn(
                        "Failed to serialize embedding mappings for {}/{}",
                        scriptResult.dbName(),
                        scriptResult.dbVersion(),
                        e);
            }
        }
        ...
        events.sendEvent(
                benchmarkId,
                SseEvents.EVENT_SCRIPT_GENERATED,
                Map.of(
                        SseEvents.PAYLOAD_BENCHMARK_ID, benchmarkId,
                        SseEvents.PAYLOAD_DATABASE_ID, db.getId(),
                        SseEvents.PAYLOAD_SCRIPT_PREVIEW,
                                db.getScript()
                                        .substring(0, Math.min(db.getScript().length(), 500))));
    }
    ...
    private void startContainers(String benchmarkId) {
        ...
        List<BenchmarkDatabase> readyDatabases =
                benchmark.getDatabases().stream()
                        .filter(db -> db.getStatus() == DatabaseStatus.SCRIPT_READY)
                        .toList();

        List<? extends Future<?>> futures =
                readyDatabases.stream()
                        .map(
                                db ->
                                        executor.submit(
                                                () ->
                                                        startSingleContainer(
                                                                benchmarkId, db.getId())))
                        .toList();
    ...
    private void startSingleContainer(String benchmarkId, String dbId) {
        ...
        try {
            ...
            String image =
                    db.getDockerImage() != null
                            ? db.getDockerImage()
                            : engine.dockerImage(db.getDbVersion());
            String containerNameRoot =
                    "benchmark-" + benchmarkId.substring(0, 8) + "-" + db.getDbName();
            ...
            databaseRepository.save(db);
            events.broadcastDatabaseStatus(
                    benchmarkId, db.getId(), DatabaseStatus.FAILED, e.getMessage());
        }
    }
    ...
    private void initializeSingleDatabase(String benchmarkId, String dbId) {
        ...
        try {
            scriptExecutor.executeScript(
                    db.getContainerId(), db.getDbName(), db.getScript(), db.getHostPort());
    ...
    private void captureBaseline(String benchmarkId, String dbId) {
        ...
        if (bytes == null) {
            log.warn(
                    "Baseline capture skipped for {} ({}): probe unavailable",
                    db.getDbName(),
                    dbId);
            return;
        }
        ...
        databaseRepository.save(db);
        events.sendEvent(
                benchmarkId, SseEvents.EVENT_DATABASE_SIZE_DIRTY, Map.of("databaseId", dbId));
    }
    ...
    private void failRemainingDatabases(String benchmarkId, String errorMessage) {
        Benchmark benchmark = benchmarkRepository.findById(benchmarkId).orElseThrow();
        for (BenchmarkDatabase db : benchmark.getDatabases()) {
            if (db.getStatus() != DatabaseStatus.FAILED
