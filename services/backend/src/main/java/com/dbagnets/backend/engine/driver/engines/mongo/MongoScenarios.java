...
public final class MongoScenarios {
    ...
    private static Map<String, Long> executeStandaloneAggregate(
            MongoDatabase db, String childEntity, String fkColumn) {
        String collectionName = childEntity.toLowerCase();
        String groupField = fkColumn.toLowerCase();
        MongoCollection<Document> collection = db.getCollection(collectionName);
        AggregateIterable<Document> pipeline =
                collection.aggregate(
                        Arrays.asList(
                                new Document(
                                        "$match",
                                        new Document(groupField, new Document("$ne", null))),
                                new Document(
                                        "$group",
    ...
    private static Map<String, Long> executeEmbeddedAggregate(
            MongoDatabase db, EmbeddingMapping mapping) {
    ...
    public static long executeRangeCount(
        ...
        if (!ScenarioSupport.isNumericLike(attr)) {
            throw new IllegalArgumentException(
                    "Attribute " + attribute + " is not numeric — type " + attr.dataType());
        }
    ...
    public static List<String> executeTraversal(
        ...
        List<PgScenarios.TraversalLevel> chain =
                PgScenarios.resolveChain(schema, startEntity, depth);
        if (chain.isEmpty()) return List.of();

        java.util.Set<String> reachable = new java.util.TreeSet<>();
        com.dbagnets.backend.engine.driver.support.FrontierBfs.descend(
                chain,
                startLogicalId,
