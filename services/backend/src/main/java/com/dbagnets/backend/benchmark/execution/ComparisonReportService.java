...
import com.dbagnets.backend.benchmark.model.ComparisonReportResponse.DatabaseDescriptor;
import com.dbagnets.backend.benchmark.model.ComparisonReportResponse.DeleteSummary;
import com.dbagnets.backend.benchmark.model.ComparisonReportResponse.InsertSummary;
import com.dbagnets.backend.benchmark.model.ComparisonReportResponse.RadarScore;
import com.dbagnets.backend.benchmark.model.ComparisonReportResponse.ReadSummary;
...
public class ComparisonReportService {
    ...
    public ComparisonReportResponse build(String benchmarkId) {
        ...
        List<DatabaseDescriptor> descriptors = databases.stream()
                .map(db -> new DatabaseDescriptor(
                        db.getId(),
                        db.getDbName(),
                        db.getDbVersion(),
                        db.getDbType().name()))
        ...
        List<BenchmarkRun> allRuns = runRepository.findByBenchmarkIdOrderByCreatedAtDesc(benchmarkId);

        List<InsertSummary> insertSummaries = buildInsertSummaries(databases, filterByType(allRuns, OperationType.INSERT));
        List<ReadSummary> readSummaries = buildReadSummaries(databases, filterByType(allRuns, OperationType.READ));
        List<DeleteSummary> deleteSummaries = buildDeleteSummaries(databases, filterByType(allRuns, OperationType.DELETE));

        List<RadarScore> radar = buildRadarScores(databases, insertSummaries, readSummaries, deleteSummaries);
    ...
    private List<InsertSummary> buildInsertSummaries(List<BenchmarkDatabase> databases, List<BenchmarkRun> runs) {
        ...
        for (BenchmarkDatabase db : databases) {
            ...
            long conflicts = results.stream()
                    .mapToInt(r -> r.getConflictsSkipped() == null ? 0 : r.getConflictsSkipped())
                    .sum();
    ...
    private List<ReadSummary> buildReadSummaries(List<BenchmarkDatabase> databases, List<BenchmarkRun> runs) {
        Map<String, List<BenchmarkResult>> perDb = groupResultsByDatabase(runs);
        List<ReadSummary> out = new ArrayList<>();
        for (BenchmarkDatabase db : databases) {
            List<BenchmarkResult> results = perDb.getOrDefault(db.getId(), List.of());
            long totalSamples = results.stream()
                    .mapToLong(r -> r.getSamplesRecorded() == null ? 0L : r.getSamplesRecorded())
                    .sum();
            int success = countStatus(results, RunStatus.SUCCESS);
            int failed = countStatus(results, RunStatus.FAILED);
            out.add(new ReadSummary(
                    db.getId(),
                    db.getDbName(),
                    results.size(),
                    totalSamples,
    ...
    private List<DeleteSummary> buildDeleteSummaries(List<BenchmarkDatabase> databases, List<BenchmarkRun> runs) {
        Map<String, List<BenchmarkResult>> perDb = groupResultsByDatabase(runs);
        List<DeleteSummary> out = new ArrayList<>();
        for (BenchmarkDatabase db : databases) {
            List<BenchmarkResult> results = perDb.getOrDefault(db.getId(), List.of());
            long totalRows = sumLong(results, BenchmarkResult::getRowsAffected);
            long sizeFreed = results.stream()
                    .filter(r -> r.getDataSizeBefore() != null && r.getDataSizeAfter() != null)
                    .mapToLong(r -> Math.max(0L, r.getDataSizeBefore() - r.getDataSizeAfter()))
                    .sum();
            int success = countStatus(results, RunStatus.SUCCESS);
            int failed = countStatus(results, RunStatus.FAILED);
            out.add(new DeleteSummary(
                    db.getId(),
                    db.getDbName(),
                    results.size(),
                    totalRows,
    ...
    private List<RadarScore> buildRadarScores(List<BenchmarkDatabase> databases,
        ...
        deletes.forEach(s -> sizeFreedByDb.put(s.databaseId(),
                s.totalSizeFreedBytes() == null ? 0.0 : (double) s.totalSizeFreedBytes()));
    ...
    private int totalSuccessFor(String dbId, List<InsertSummary> i, List<ReadSummary> r, List<DeleteSummary> d) {
        int sum = 0;
    ...
    private int totalRunsFor(String dbId, List<InsertSummary> i, List<ReadSummary> r, List<DeleteSummary> d) {
        int sum = 0;
    ...
    private long sumLong(List<BenchmarkResult> results, java.util.function.Function<BenchmarkResult, Long> getter) {
        long sum = 0L;
    ...
    private Double averageNs(List<BenchmarkResult> results,
    ...
    private double normalizeLowerBetter(Map<String, Double> values, String key) {
        ...
        double min = values.values().stream()
                .filter(v -> v > 0.0)
