...
public enum DatabaseEngine {
    ...
    TIMESCALEDB(
            "timescaledb",
            5432,
            v -> "timescale/timescaledb:latest-pg17",
            Map.of("POSTGRES_PASSWORD", "benchmark", "POSTGRES_DB", "benchmark")),
    MYSQL(
            "mysql",
            3306,
    ...
    MEMGRAPH("memgraph", 7687, v -> "memgraph/memgraph:" + v, Map.of()),
    MONGODB("mongodb", 27017, v -> "mongo:" + v, Map.of()),
    REDIS("redis", 6379, v -> "redis:" + v, Map.of()),
    ARANGODB("arangodb", 8529, v -> "arangodb:" + v, Map.of("ARANGO_ROOT_PASSWORD", "root")),
    ELASTICSEARCH(
            "elasticsearch",
            9200,
            v -> "docker.elastic.co/elasticsearch/elasticsearch:" + v + ".0",
            Map.of(
                    "discovery.type",
                    "single-node",
    ...
    QUESTDB("questdb", 8812, v -> "questdb/questdb:" + v, Map.of("QDB_CAIRO_COMMIT_LAG", "1000")),
    DYNAMODB("dynamodb", 8000, v -> "amazon/dynamodb-local:latest", Map.of()),
    ETCD(
            "etcd",
            2379,
            v -> "quay.io/coreos/etcd:v" + v,
            Map.of(
    ...
    DatabaseEngine(
            String dbName,
            int port,
            Function<String, String> imageBuilder,
    ...
    public static DatabaseEngine of(String dbName) {
        return Arrays.stream(values())
                ...
                .findFirst()
                .orElseThrow(
                        () -> new IllegalArgumentException("Unknown database engine: " + dbName));
    }
}
