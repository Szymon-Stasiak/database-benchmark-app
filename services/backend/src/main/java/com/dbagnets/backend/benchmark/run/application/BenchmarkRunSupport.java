...
public class BenchmarkRunSupport {
    ...
    private static final TypeReference<List<ResourceSample>> RESOURCE_SAMPLES_TYPE =
            new TypeReference<>() {};
    ...
    public LogicalSchema loadSchema(Benchmark benchmark) {
        if (benchmark.getLogicalSchema() == null) {
            throw new IllegalStateException(
                    "Benchmark " + benchmark.getId() + " has no logical schema");
        }
    ...
    public List<ResourceSample> loadResourceTimeline(
            String runId, String resultId, OperationType expected) {
        BenchmarkRun run =
                runRepository
                        .findById(runId)
        ...
        if (run.getOperationType() != expected) {
            throw new IllegalArgumentException(
                    "Run " + runId + " is not a " + expected.name() + " run");
        }
        BenchmarkResult result =
                resultRepository
                        .findById(resultId)
                        .orElseThrow(
                                () -> new NoSuchElementException("Result not found: " + resultId));
        if (!result.getRun().getId().equals(runId)) {
            throw new IllegalArgumentException(
                    "Result " + resultId + " does not belong to run " + runId);
        }
    ...
    public record EntityRunContext(
            Benchmark benchmark, List<String> selectedIds, RunPreview preview) {}
    ...
    public BenchmarkRun createEntityRun(
            String benchmarkId,
            OperationType type,
            String entityName,
            ...
            Consumer<BenchmarkRun> broadcast) {
    ...
    public record DatabaseContext(
            BenchmarkDatabase db, EngineDriver driver, EmbeddingMap embeddings) {}
    ...
    public Optional<DatabaseContext> resolveDatabaseContext(
        ...
        EmbeddingMap embeddings =
                EmbeddingMap.from(embeddingMappingLoader.parse(db.getEmbeddingMappings()));
    ...
    public Optional<List<EntityIdRegistry.RegistryEntry>> resolveTargets(
            ...
            String resultId,
            Consumer<BenchmarkResult> skipBroadcast) {
        List<EntityIdRegistry.RegistryEntry> targets =
                entityIdRegistry.lookupEntries(databaseId, entityName, selectedLogicalIds);
        if (targets.isEmpty()) {
            String dbName =
                    resultRepository
                            .findById(resultId)
                            .map(BenchmarkResult::getDbName)
                            .orElse(databaseId);
    ...
    public Optional<EngineDriver> resolveDriver(
            BenchmarkDatabase db, String resultId, Consumer<BenchmarkResult> skipBroadcast) {
    ...
    public void persistResourceSummary(
            String resultId, ResourceMetricsSummary summary, Consumer<BenchmarkResult> broadcast) {
    ...
    public static BenchmarkDatabase findDatabase(Benchmark benchmark, String databaseId) {
        return benchmark.getDatabases().stream()
                ...
                .orElseThrow(
                        () ->
                                new NoSuchElementException(
                                        "Database not in benchmark: " + databaseId));
    }
