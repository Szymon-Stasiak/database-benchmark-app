package com.dbagnets.backend.benchmark.driver.arango;
...
public class ArangoDriver implements EngineDriver {
    ...
    private static final String AUTH = "Basic " + Base64.getEncoder()
            .encodeToString("root:root".getBytes(StandardCharsets.UTF_8));
    ...
    public TimedOperation insert(InsertContext ctx) {
        ...
        for (CascadeNode node : ctx.plan().nodesInInsertOrder()) {
            ...
            EntityOutcome outcome = bulkInsert(client, ctx, node, entity, collection, rows);
    ...
    public TimedOperation read(ReadContext ctx) {
        ...
        for (int i = 0; i < ctx.targets().size(); i++) {
            ...
            try {
                ...
                if (resp != null && resp.hasNonNull("_key")) rowsRead++;
            } catch (Exception ex) {
                log.warn("Arango read failed {}/{}: {}", collection, entry.physicalId(), ex.getMessage());
            }
        }
        long wireTimeNs = System.nanoTime() - wireStart;
    ...
    public TimedOperation delete(DeleteContext ctx) {
        ...
        for (int i = 0; i < ctx.targets().size(); i++) {
            ...
            try {
                ...
                samples[i] = sampleNs;
                totalDbTimeNs += sampleNs;
                if (resp != null && resp.hasNonNull("_key")) rowsAffected++;
            } catch (Exception ex) {
                log.warn("Arango delete failed {}/{}: {}", collection, entry.physicalId(), ex.getMessage());
            }
        }
        long wireTimeNs = System.nanoTime() - wireStart;
    ...
    private void ensureCollection(WebClient client, String collection) {
        try {
            client.post()
                    .uri("/_db/_system/_api/collection")
                    .contentType(MediaType.APPLICATION_JSON)
                    .bodyValue(Map.of("name", collection, "type", 2))
                    .retrieve()
                    .bodyToMono(JsonNode.class)
                    .timeout(Duration.ofSeconds(15))
    ...
    private EntityOutcome bulkInsert(WebClient client,
        ...
        for (int from = 0; from < rows.size(); from += batchSize) {
            ...
            try {
                long start = System.nanoTime();
                client.post()
                        .uri(uri -> uri.path("/_db/_system/_api/document/{c}")
                                .queryParam("overwriteMode", "ignore")
                                .build(collection))
                        .contentType(MediaType.APPLICATION_JSON)
                        .bodyValue(docs)
                        .retrieve()
                ...
                log.warn("Arango bulk insert failed on {} batch {}: {}", collection, batchIndex, ex.getMessage());
            }
            batchIndex++;
    ...
    private Map<String, Object> toDocument(LogicalEntity entity, GeneratedRow row) {
        ...
        entity.primaryKey().ifPresent(pk -> doc.put("_key", String.valueOf(row.get(pk.name()))));
