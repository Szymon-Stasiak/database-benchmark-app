...
public class PgDriver extends AbstractSqlDriver {
    ...
    private static final SqlCascadeDeleter.Dialect DIALECT =
            new SqlCascadeDeleter.Dialect(
                    ident -> "\"" + ident.replace("\"", "\"\"") + "\"", PgValueBinder::bind);
                                ...
                                case RangeParams p -> {
                                    long count =
                                            PgScenarios.executeRangeCount(
                                                    ...
                                                    p.min(),
                                                    p.max());
                                    yield ResultCanonicalizer.build(
                                            java.util.Map.of("count", count), count);
                                }
                                case TraversalParams p -> {
                                    var ids =
                                            PgScenarios.executeTraversal(
                                                    conn,
                                                    ctx.schema(),
                                                    p.startEntity(),
                                                    p.startLogicalId(),
    ...
    private List<PgScenarios.TraversalLevel> childChainForRead(
            LogicalSchema schema, String entityName, ReadDepth depth) {
    ...
    private long fetchDescendants(
            Connection conn,
            LogicalSchema schema,
            Object rootId,
            List<PgScenarios.TraversalLevel> chain)
            throws SQLException {
        return FrontierBfs.descendSql(
                chain,
                rootId,
