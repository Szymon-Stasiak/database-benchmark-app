package com.dbagnets.backend.engine.driver.engines.influx;
...
public class InfluxdbDriver implements EngineDriver {
                ...
                entry -> {
                    ...
                    String body =
                            client.post()
                                    ...
                                    .block();
                    return body != null && body.lines().anyMatch(l -> l.contains(measurement))
                            ? 1
                            : 0;
                });
    }
        ...
        return PerTargetLoop.run(
                ...
                entry -> {
                    ...
                    client.post()
                            .uri(
                                    uri ->
                                            uri.path("/api/v2/delete")
                                                    .queryParam("org", ORG)
                                                    .queryParam("bucket", BUCKET)
                                                    .build())
                            .contentType(MediaType.APPLICATION_JSON)
                            .bodyValue(
                                    "{\"start\":\"1970-01-01T00:00:00Z\",\"stop\":\"2100-01-01T00:00:00Z\",\"predicate\":\""
                                            + predicate
                                            + "\"}")
                            .retrieve()
                            .toBodilessEntity()
    ...
    private EntityOutcome writePoints(
            WebClient client, InsertContext ctx, CascadeNode node, List<GeneratedRow> rows)
            throws Exception {
