...
public class InsertRunOrchestrator extends AbstractRunOrchestrator {
    ...
    protected String runStatusEventName() {
        return SseEvents.EVENT_INSERT_RUN_STATUS;
    }

    @Override
    protected String resultStatusEventName() {
        return SseEvents.EVENT_INSERT_RESULT_STATUS;
    }
    ...
    public BenchmarkRun startRun(String benchmarkId, StartInsertRunRequest request) {
        ...
        run.setWorkerCount(
                BenchmarkRunSupport.orDefault(request.workerCount(), DEFAULT_WORKER_COUNT));
        ...
        wrapExecute(
                ...
                () -> {
                    ...
                    Map<String, List<GeneratedRow>> rowsByEntity =
                            recordBuilder.generateAll(schema, plan, vault);
    ...
    private void persistSuccess(
            String benchmarkId, String runId, String resultId, TimedOperation timed) {
        transactionTemplate.executeWithoutResult(
                s -> {
                    ...
                    if (!timed.recordedIds().isEmpty()) {
                        Map<String, List<EntityIdRegistry.RegistryEntry>> byEntity =
                                new HashMap<>();
                        for (RecordedId id : timed.recordedIds()) {
                            byEntity.computeIfAbsent(id.entityName(), k -> new ArrayList<>())
                                    .add(
                                            new EntityIdRegistry.RegistryEntry(
                                                    id.logicalId(), id.physicalId()));
                        }
                        String benchmarkIdLocal = r.getRun().getBenchmarkId();
                        String databaseIdLocal = r.getDatabaseId();
                        for (Map.Entry<String, List<EntityIdRegistry.RegistryEntry>> entry :
                                byEntity.entrySet()) {
                            registry.recordAll(
                                    benchmarkIdLocal,
                                    databaseIdLocal,
