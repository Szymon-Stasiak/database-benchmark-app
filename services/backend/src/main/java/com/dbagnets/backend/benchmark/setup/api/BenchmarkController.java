...
public class BenchmarkController {
    ...
    public ResponseEntity<BenchmarkResponse> createBenchmark(
            @Valid @RequestBody CreateBenchmarkRequest request, @CurrentUser User user) {
        return ResponseEntity.status(201).body(lifecycle.createBenchmark(request, user));
    }

    @PostMapping(value = "/import", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<BenchmarkResponse> importBenchmark(
            @RequestParam("file") MultipartFile file, @CurrentUser User user) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Failed to read uploaded bundle: " + e.getMessage(), e);
        }
    ...
    public ResponseEntity<Void> redeployDatabase(
            @PathVariable String id, @PathVariable String dbId) {
    ...
    public ResponseEntity<Void> restartDatabase(
            @PathVariable String id, @PathVariable String dbId) {
    ...
    public LogsResponse getDatabaseLogs(
            @PathVariable String dbId, @RequestParam(defaultValue = "200") int tailLines) {
        return new LogsResponse(operations.getDatabaseLogs(dbId, tailLines));
    }

    @GetMapping(
            value = "/{id}/databases/{dbId}/logs/stream",
                    ...
                    SseEmitter.event()
                            ...
                            .data(
                                    objectMapper.writeValueAsString(
                                            SseEvents.benchmarkStatusPayload(
                                                    benchmarkId, benchmark.status()))));
            for (BenchmarkResponse.DatabaseResponse db : benchmark.databases()) {
                emitter.send(
                        SseEmitter.event()
                                ...
                                .data(
                                        objectMapper.writeValueAsString(
                                                SseEvents.databaseStatusPayload(
                                                        benchmarkId,
                                                        db.id(),
                                                        db.status(),
                                                        db.errorMessage()))));
                if (db.hostPort() != null) {
                    emitter.send(
                            SseEmitter.event()
                                    ...
                                    .data(
                                            objectMapper.writeValueAsString(
                                                    SseEvents.databasePortAssignedPayload(
                                                            benchmarkId, db.id(), db.hostPort()))));
                }
            }
        } catch (Exception e) {
            log.warn(
                    "Failed to push initial SSE state for benchmark {}: {}",
                    benchmarkId,
                    e.toString());
        }
    }
