...
class ScenarioApplicabilityTest {
    ...
    void aggregateCoversRelationalDocumentAndGraph() {
        Set<DatabaseEngine> applicable =
                ScenarioApplicability.applicableEngines(ScenarioType.AGGREGATE_GROUP_COUNT);
        assertThat(applicable)
                .contains(DatabaseEngine.POSTGRESQL, DatabaseEngine.MONGODB, DatabaseEngine.NEO4J);
    }

    @Test
    void rangeFilterCoversRelationalDocumentAndGraph() {
        Set<DatabaseEngine> applicable =
                ScenarioApplicability.applicableEngines(ScenarioType.RANGE_FILTER);
        assertThat(applicable)
                .contains(DatabaseEngine.POSTGRESQL, DatabaseEngine.MONGODB, DatabaseEngine.NEO4J);
    }
    ...
    void traversalCoversRelationalDocumentAndGraph() {
        Set<DatabaseEngine> applicable =
                ScenarioApplicability.applicableEngines(ScenarioType.GRAPH_TRAVERSAL);
        assertThat(applicable)
                .contains(DatabaseEngine.POSTGRESQL, DatabaseEngine.MONGODB, DatabaseEngine.NEO4J);
    }
    ...
    void knnRestrictedToVectorEngines() {
        Set<DatabaseEngine> applicable =
                ScenarioApplicability.applicableEngines(ScenarioType.VECTOR_KNN);
    ...
    void postgresqlDoesNotAdvertiseKnn() {
        assertThat(
                        ScenarioApplicability.isApplicable(
                                ScenarioType.VECTOR_KNN, DatabaseEngine.POSTGRESQL))
                .isFalse();
    }
}
